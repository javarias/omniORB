...
final class CallInterceptor_impl extends org.omg.CORBA.LocalObject
    implements ClientRequestInterceptor
{
    ...
    void
    testArgs(ClientRequestInfo ri, boolean resultAvail)
    {
	String op = ri.operation();
        ...
        else if(op.startsWith("one_"))
        {
            ...
            if(which.startsWith("string"))
                isstr = true;
            else // if(which.startsWith("struct"))
                isstr = false;
	...
	try
	{
	    factory = org.omg.IOP.CodecFactoryHelper.narrow(
	...
	org.omg.IOP.Encoding how = new org.omg.IOP.Encoding(
	    (byte)org.omg.IOP.ENCODING_CDR_ENCAPS.value, (byte)0, (byte)0);
	...
	//
        ...
        //
        org.omg.IOP.TaggedComponent componentEncoding =
            ri.get_effective_component(MY_COMPONENT_ID.value);
        byte[] componentData = componentEncoding.component_data;
        Any componentAny = null;
	try
	{
	    componentAny = cdrCodec_.decode_value(
		componentData,MyComponentHelper.type());
	}
	...
	catch(org.omg.IOP.CodecPackage.FormatMismatch ex)
	{
        ...
        {
            org.omg.IOP.ServiceContext sc =
                ri.get_reply_service_context(REQUEST_CONTEXT_ID.value);
            TEST(false);
        }
        catch(BAD_INV_ORDER ex)
        {
	    ...
	    System.arraycopy(data, 0, sc.context_data, 0, data.length);
            ...
            try
            {
                org.omg.IOP.ServiceContext sc2 =
                    ri.get_request_service_context(REQUEST_CONTEXT_ID.value);
            }
            catch(BAD_PARAM ex)
            {
	...
	{
	    componentAny =
		cdrCodec_.decode_value(componentData,
	...
	Policy policy = ri.get_request_policy(MY_CLIENT_POLICY_ID.value);
        ...
        if(op.equals("test_service_context"))
        {
           ...
           try
            ...
            {
                org.omg.IOP.ServiceContext sc =
                    ri.get_reply_service_context(REPLY_CONTEXT_ID.value);
                TEST(sc.context_id == REPLY_CONTEXT_ID.value);
		...
		try
		{
		    any =cdrCodec_.decode_value(data,
						ReplyContextHelper.type());
		}
		catch(org.omg.IOP.CodecPackage.FormatMismatch ex)
		{
		    TEST(false);
		...
		catch(org.omg.IOP.CodecPackage.TypeMismatch ex)
		{
		    TEST(false);
                ...
                TEST(context.data.equals("reply"));
                TEST(context.val == 20);
            }
            catch(BAD_PARAM ex)
            {
	...
	catch(org.omg.IOP.CodecPackage.FormatMismatch ex)
	{
        ...
        {
            org.omg.IOP.ServiceContext sc =
                ri.get_request_service_context(REPLY_CONTEXT_ID.value);
            TEST(false);
        }
        catch(BAD_PARAM ex)
        {
    ...
    receive_exception(ClientRequestInfo ri)
    {
        ...
        // valid
        String op = ri.operation();
        TEST(op.equals("systemexception") ||
             op.equals("userexception") ||
	...
	catch(org.omg.IOP.CodecPackage.TypeMismatch ex)
	{
        ...
        try
        {
            ...
            if(!user)
            {
		SystemException ex = test.common.Util.unmarshalSystemException(
		    rc.create_input_stream());
            }
            else
            {
