...
MyApp::initServer(int argc, char** argv)
{
  ...
  POA_multiEcho_tie<multiEcho_i> *mytieobj = new POA_multiEcho_tie<multiEcho_i>(*myobj);
  PortableServer::ObjectId_var server_id = poa()->activate_object(mytieobj);
  CORBA::Object_var server_obj = mytieobj->_this();
  mytieobj->_remove_ref();
